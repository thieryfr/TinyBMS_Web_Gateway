...
static void wifi_start_ap_mode(void)
{
    ...
    strlcpy((char *)ap_config.ap.ssid, CONFIG_TINYBMS_WIFI_AP_SSID, sizeof(ap_config.ap.ssid));
    ap_config.ap.ssid_len = strlen((const char *)ap_config.ap.ssid);
    ap_config.ap.channel = CONFIG_TINYBMS_WIFI_AP_CHANNEL;
    ap_config.ap.max_connection = CONFIG_TINYBMS_WIFI_AP_MAX_CLIENTS;
    ap_config.ap.beacon_interval = 100;

    size_t password_len = strlen(CONFIG_TINYBMS_WIFI_AP_PASSWORD);
    ...
    if (password_len > 0) {
        strlcpy((char *)ap_config.ap.password,
                CONFIG_TINYBMS_WIFI_AP_PASSWORD,
...
static void wifi_configure_sta(void)
{
    ...
    strlcpy((char *)sta_config.sta.ssid, CONFIG_TINYBMS_WIFI_STA_SSID, sizeof(sta_config.sta.ssid));
    strlcpy((char *)sta_config.sta.password, CONFIG_TINYBMS_WIFI_STA_PASSWORD, sizeof(sta_config.sta.password));

    size_t password_len = strlen(CONFIG_TINYBMS_WIFI_STA_PASSWORD);
    if (password_len > 0 && password_len < 8) {
    ...
    if (s_sta_netif != NULL && strlen(CONFIG_TINYBMS_WIFI_STA_HOSTNAME) > 0) {
        esp_err_t err = esp_netif_set_hostname(s_sta_netif, CONFIG_TINYBMS_WIFI_STA_HOSTNAME);
        if (err != ESP_OK) {
    ...
    if (event_base == WIFI_EVENT) {
        switch (event_id) {
            case WIFI_EVENT_STA_START:
                ...
                ESP_LOGI(TAG, "Wi-Fi station started, connecting to '%s'", CONFIG_TINYBMS_WIFI_STA_SSID);
                wifi_attempt_connect();
                break;
            case WIFI_EVENT_STA_CONNECTED:
                ESP_LOGI(TAG, "Wi-Fi connected to '%s'", CONFIG_TINYBMS_WIFI_STA_SSID);
                s_retry_count = 0;
            ...
            case WIFI_EVENT_STA_DISCONNECTED: {
                ...
                if (s_retry_count < CONFIG_TINYBMS_WIFI_STA_MAX_RETRY) {
                    s_retry_count++;
                    ESP_LOGW(TAG, "Wi-Fi disconnected, retry %d/%d", s_retry_count, CONFIG_TINYBMS_WIFI_STA_MAX_RETRY);
                    wifi_attempt_connect();
                } else {
                    ESP_LOGE(TAG, "Wi-Fi failed to connect after %d attempts", CONFIG_TINYBMS_WIFI_STA_MAX_RETRY);
#if CONFIG_TINYBMS_WIFI_AP_FALLBACK
                    wifi_start_ap_mode();
#else
                    s_retry_count = 0;
...
void wifi_init(void)
{
    ...
    if (strlen(CONFIG_TINYBMS_WIFI_STA_SSID) == 0) {
#if CONFIG_TINYBMS_WIFI_AP_FALLBACK
