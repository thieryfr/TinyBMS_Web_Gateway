...
TEST_CASE("uart_bms_process_frame publishes event and notifies listeners", "[uart_bms]")
{
    ...
    uint8_t frame[128] = {0};
    size_t frame_len = build_frame(frame, sizeof(frame), kSampleValues, UART_BMS_MAX_REGISTERS);
    ...
    TEST_ASSERT_EQUAL(UART_BMS_EVENT_ID_LIVE_DATA, event.id);
    ...
    TEST_ASSERT_TRUE(s_listener_data.pack_current_valid);
...
TEST_CASE("uart_bms_decode_frame marks missing fields as invalid", "[uart_bms]")
{
    uint8_t frame[64] = {0};
    const uint16_t partial_values[] = {6000, (uint16_t)(int16_t)-50};
    size_t frame_len = build_frame(frame, sizeof(frame), partial_values, ARRAY_SIZE(partial_values));

    uart_bms_live_data_t decoded = {0};
    TEST_ASSERT_EQUAL(ESP_OK, uart_bms_decode_frame(frame, frame_len, &decoded));

    TEST_ASSERT_TRUE(decoded.pack_voltage_valid);
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 60.0f, decoded.pack_voltage_v);

    TEST_ASSERT_TRUE(decoded.pack_current_valid);
    TEST_ASSERT_FLOAT_WITHIN(0.01f, -5.0f, decoded.pack_current_a);

    TEST_ASSERT_FALSE(decoded.min_cell_mv_valid);
    TEST_ASSERT_FALSE(decoded.state_of_charge_valid);
    TEST_ASSERT_FALSE(decoded.uptime_valid);
}
